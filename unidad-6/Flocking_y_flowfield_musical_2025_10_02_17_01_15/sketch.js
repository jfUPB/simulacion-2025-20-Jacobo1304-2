// sketch.js — Integración final lista para usar
...
// -----------------------------
// Background alpha mapping por preset
// -----------------------------
// preset 1: alpha 60 (trail moderado)
// preset 2: alpha 30 (trail más largo)
// preset 3: alpha 255 (sin alpha = fondo opaco, sin trails)
const presetBackgroundAlpha = {
  1: 60,
  2: 30,
  3: 10
};
...
// -----------------------------
function draw() {
  // background con alpha según preset seleccionado
  const alphaVal = presetBackgroundAlpha[currentPreset] !== undefined ? presetBackgroundAlpha[currentPreset] : 200;
  // usamos HSB: background(hue, sat, bri, alpha)
...
function applyPreset(n) {
  if (flock && typeof flock.applyPreset === 'function') {
    flock.applyPreset(n);
    currentPreset = n;
    // cuando cambiamos preset, si preset 3 (gris), actualizar visual de fondo inmediato
    if (typeof statusEl !== 'undefined' && statusEl) {
      statusEl.textContent = `Preset ${n} aplicado`;
    }
  }
}
